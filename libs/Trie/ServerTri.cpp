...
public:
    ...
    void insert(const std::vector<bool> &ipBits, ft::shared_ptr<VirtualServer> curVirtualServer, int idx = 0) {
        if (idx == ipBits.size()) {
            _type = Server;
            this->_data = curVirtualServer;
        } else {
            _next[ipBits[idx]].insert(ipBits, curVirtualServer, idx + 1);
        }
    }

    ft::shared_ptr<VirtualServer> getData(const std::vector<bool> &ipBits, int idx = 0) throw (std::exception) {
        if (idx == ipBits.size()) {
            if (isHasData()) {
                return _data;
            } else {
...
std::vector<bool> convertIPToVector(const std::string& ip) {
    ...
    void _longestPrefix(const std::vector<bool> &ipBits, int idx, std::vector<bool> &current, 
	ft::shared_ptr<VirtualServer> &result) {
        if (isHasData()) {
            result = this->_data;
        }
