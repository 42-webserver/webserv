...
ClientMaxBodySizeElement::ClientMaxBodySizeElement(const ClientMaxBodySizeElement &ref)
    : ConfigElement(ref), _num(ref._num), _unit(ref._unit) {}
...
bool ClientMaxBodySizeElement::_parse(std::ifstream &infile) throw(std::exception) {
	...
	this->_num = ('0' <= last && last <= '9') ? std::atoi(token.c_str())
	                                           : std::atoi(token.substr(0, token.size() - 1).c_str());
	if (this->_numIsNotValid(this->_num)) {
		throw (InvalidArgumentException());
	}
	this->_unit = ('0' <= last && last <= '9') ? 'B' : last;
